...
head(org)
boxplot(npvs[-which(as.character(npvs) == "NA")])
orgdf  <-  as.data.frame(matrix(unlist(org), byrow = TRUE,ncol=27))
names(orgdf) = names
orgdf[1,]$NPV == npvs[1]
orgdf[1000,]$NPV == npvs[1000]
write.csv(orgdf, file = "Github/Wang499/sampleNPV.txt", sep = "|")
test = read.csv(file = "Github/Wang499/sampleNPV.txt", header = TRUE)
test$X = NULL