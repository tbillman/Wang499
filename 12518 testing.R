...
q = q*1:length(q)
q =c(0, q[q>0]); q = q + 1
tes = sapply(1:length(q),function(x){
  as.character(perf$`Sequence Number`[q[x]]) == as.character(org$`Loan Sequence Number`[x])
})
bad= which(tes == F)
unique(perf$`Sequence Number`)[bad[1]]

perf = rbind(perf[1:(q[bad[1]]-1),], perf[q[bad[1]+1]:dim(perf)[2],])
tes = sapply(1:length(q),function(x){
  as.character(perf$`Sequence Number`[q[x]]) == as.character(org$`Loan Sequence Number`[x])
})
bad = which(tes == F)
